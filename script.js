...
document.querySelectorAll('.glitchable, .btn, nav a, .pill').forEach(el=>{
...
closeModal?.addEventListener('click', ()=>{ closeModalFn(); glitchBurst(90); });
document.getElementById('modalBackdrop')?.addEventListener('click', ()=>{ closeModalFn(); glitchBurst(90); });
...
/* Bot icon handler (non crasha se botReady non esiste) */
const botIcon = document.getElementById('bot-icon');
if (botIcon) {
  botIcon.addEventListener('click', () => {
    const ready = (typeof window.botReady === 'undefined') ? true : !!window.botReady;
    if (!ready) return console.warn("Bot non ancora pronto");
